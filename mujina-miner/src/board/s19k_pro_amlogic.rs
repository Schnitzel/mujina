...
///
/// Holds one or more `SelectedHashboard`s — the Amlogic A113D control
/// board carries up to three hashboards on the same APW12 / SoC, and
/// `select_present_hashboards` picks every slot whose detect GPIO
...
impl Board for S19kProAmlogic {
    ...
    async fn create_hash_threads(&mut self) -> Result<Vec<Box<dyn HashThread>>, BoardError> {
        ...
        for selected in self.selected_hashboards.clone() {
            ...
            let thread_hashrate = u64::from(inner_thread.capabilities().hashrate_estimate);
